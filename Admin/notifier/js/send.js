...
function onformend(data) {
  ...
  function forward() {
    modal.alert('Sending messages', function(main, btn) {
      ...
      data.users.forEach(function(ph) {
        app.sendWhatsappMsg(ph.replace('+', ''), ("*" + data.title + "* \n\n" + data.des)).then(() => {
          bushido.realtime
            ...
            .then(function() {
              sended += 1;
              id('loaderlog').innerHTML = sended + ' Sended ( total: ' + data.users.length + ' )'

              if (sended == data.users.length) {
                id(btn).click();
                location.reload();
              }
            })
      ...
      return '<center><img src="../../assets/spinner/ring-resize.svg" class="svg-mini-loader loader-x2"></img><p id="loaderlog">Sending...</p></center>';
    }, '', function(divId, btnId) {
      id(btnId).style.display = 'none';
    })
  }
...
///////////////////////////////////////
// TODO: FCM Send with oath2 http v1 //
